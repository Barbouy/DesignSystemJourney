    ...
    <table class="w-full">
      <thead>
        <tr class="h-12 border-b border-neutral-100">
          <th
            v-for="column in formattedColumns" :key="column.key" class="px-4 py-0"
            :style="{ minWidth: column.minWidth, width: column.width, maxWidth: column.maxWidth }">
            <div class="flex items-center gap-2 font-medium">
              <i v-if="column.icon" :class="column.icon" class="text-lg shrink-0 text-neutral-400" />
      ...
      <tbody>
        <tr
          ...
          <td v-for="column in formattedColumns" :key="column.key" class="px-4 py-0 truncate"
            :style="{ minWidth: column.minWidth, width: column.width, maxWidth: column.maxWidth }">
